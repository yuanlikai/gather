<template>
  <div class="content">
    <Card style="border:none;margin: 16px 0;">
      <div class="ivu-page-header-title" @click="abc">发货仓用户</div>
    </Card>
...
margin-right: 16px">
      ...
      <!--<Row v-for="(item,index) in data" :key="index" style="padding: 12px 0;border-bottom: 1px solid #e8eaec;">-->
        ...
        <!--<Col span="24" v-if="item.supplierName">发货仓：{{ item.supplierName }}</Col>-->
      ...
      <div>
        <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="70">
          <FormItem v-if="userInfoType!=='SUPPLIER'" label="用户账号" prop="username">
            <Input :disabled="formValidate.id!==-1" :maxlength="36" v-model="formValidate.username"
          ...
          <FormItem v-if="formValidate.id === -1" label="发货仓" prop="supplierId">
...
<script>
  export default {
            ...
            render: (h, params) => {
              return h('div', [
                h('a', {
                  on: {
                    'click': () => {
                      this.amend(params.row.id, params.row.userInfoType, params.row.username, params.row.name, params.row.roleIds, params.row.status, params.row.allField)
                    }
                  }
                }, '修改'),
                h('Divider', {
    ...
    methods: {
      ...
      resetPass(id) {
        const _this = this;
        _this.Axios.post('/ResetPassword.ashx', {
          id:Number(id),
        }).then(res => {
          if (res.data.error === 0) {
